...
export default memo(function NatGatewayNode({ id, selected }: NodeProps) {
  const config = useArchitectureStore((s) => s.nodeConfigs[id]);
  const running = useSimulationStore((s) => s.running);
  const metrics = useSimulationStore((s) => s.nodeMetrics[id]);

  const label = config?.label ?? 'NAT Gateway';
  const bwGbps = config?.natBandwidthGbps ?? 10;
  ...
  const detail = metrics?.detail?.kind === 'nat_gateway' ? metrics.detail : null;

  const maxConnsStr = maxConns >= 1000 ? `${(maxConns / 1000).toFixed(0)}k` : String(maxConns);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '8px 12px', borderBottom: running && metrics ? '1px solid var(--border)' : 'none' }}>
        <span style={{ color: 'var(--text-dim)' }}>{bwGbps} Gbps · {maxConnsStr} conn</span>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 8px' }}>
            <div>
              ...
              <div style={{ color: COLOR, fontWeight: 600, fontSize: 12 }}>
                {metrics.rpsIn >= 1000 ? `${(metrics.rpsIn / 1000).toFixed(1)}k` : metrics.rpsIn.toFixed(0)}
              </div>
            </div>
            <div>
              ...
              <>
                <div>
                  ...
                  <div style={{ color: 'var(--text)', fontWeight: 600, fontSize: 12 }}>
                    {detail.translatedConnections >= 1000
                      ? `${(detail.translatedConnections / 1000).toFixed(1)}k`
                      : detail.translatedConnections}
                  </div>
                </div>
                <div>
                  <span style={{ color: 'var(--text-dim)', fontSize: 10 }}>BW USE</span>
                  <div
                    style={{
                      fontWeight: 600, fontSize: 12,
                      color: detail.bandwidthUtilizationPct > 80 ? 'var(--accent-orange)' : 'var(--text)',
                    }}
                  >
                    {detail.bandwidthUtilizationPct.toFixed(0)}%
                  </div>
                </div>
              </>
            )}
          </div>
          {detail && detail.droppedPackets > 0 && (
            <div
              style={{
            ...
            >
              ...
              <span style={{ fontSize: 9, fontWeight: 600, color: 'var(--accent-red)', letterSpacing: '0.04em' }}>
                {detail.droppedPackets} pkt/s dropped
              </span>
            </div>
          )}
        </div>
