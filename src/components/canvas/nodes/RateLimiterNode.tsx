...
export default memo(function RateLimiterNode({ id, selected }: NodeProps) {
  ...
  const rpsStr = rps >= 1000 ? `${(rps / 1000).toFixed(1)}k` : String(rps);

  const detail = metrics?.detail?.kind === 'rate_limiter' ? metrics.detail : null;
    ...
    >
      ...
      <div style={{ padding: '8px 12px', borderBottom: running && metrics ? '1px solid var(--border)' : 'none' }}>
        <span style={{ color: 'var(--text-dim)' }}>
          {ALGO_SHORT[algorithm] ?? algorithm} · {rpsStr} rps
        </span>
      </div>
      ...
      {running && metrics && (
        ...
        >
          ...
          <div>
            <span style={{ color: 'var(--text-dim)', fontSize: 10 }}>THROTTLE</span>
            <div
              style={{
                fontWeight: 600,
                fontSize: 12,
                color: detail && detail.throttleRate > 0.05 ? 'var(--accent-red)' : 'var(--text-dim)',
              }}
            >
              {detail ? `${(detail.throttleRate * 100).toFixed(1)}%` : '0.0%'}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div
              style={{
                fontWeight: 600,
                fontSize: 12,
                color: detail && detail.queueDepth > 0
                  ? detail.queueDepth > 400 ? 'var(--accent-red)' : 'var(--accent-orange)'
                  : 'var(--text-dim)',
              }}
            >
              {detail ? Math.round(detail.queueDepth) : 0}
            </div>
          </div>
        </div>
      )}
