...
| [ ] | **Rate Limiter** | Controls the rate of requests per client or globally to prevent abuse and protect downstream services. Key configs: limitRps, urstSize, lgorithm (token bucket / leaky bucket / fixed window). Failure mode: excess requests throttled or dropped. Stateful: tracks current token/bucket state. |
...
| [ ] | **Network Storage** | Shared filesystem storage accessible over the network (e.g. NFS, AWS EFS, Azure Files). Key configs: iops, 	hroughputMbps, latencyMs. Failure mode: IOPS exhaustion increases latency; bandwidth saturation causes errors. |
